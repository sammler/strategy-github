...
describe('profile-followers-history.bl', () => {
  ...
  it('`save` does not create a new entry for same profile_id, user_id combination (if date_to is null)', () => {
    ...
    return ProfileFollowersHistoryBL.ensure(_.clone(doc1))
      ...
      .then(() => {
        return ProfileFollowersHistoryBL.ensure(_.clone(doc2))
      })
      .then((result) => {
        expect(result).to.exist;
      ...
      .then(() => {
        return ProfileFollowersHistoryBL.count()
      })
      .then((count) => {
        expect(count).to.be.equal(1);
      })
  ...
  it('`removeByProfile` removes only the entries for the profile', () => {
    ...
    return ProfileFollowersHistoryBL.ensure(_.clone(doc1))
      ...
      .then(() => {
        return ProfileFollowersHistoryBL.removeByProfileId(doc2.profile_id)
      })
      .then(() => {
        return ProfileFollowersHistoryBL.count()
      })
  ...
  it('`getActiveFollowersByProfile` returns the active followers only', () => {
    ...
    return ProfileFollowersHistoryBL.ensure(_.clone(doc1))
      ...
      .then(() => {
        return ProfileFollowersHistoryBL.ensure(_.clone(doc3))
      })
      .then(() => {
        return ProfileFollowersHistoryBL.count()
      })
      ...
      .then(() => {
        return ProfileFollowersHistoryBL.getActiveFollowersByProfile(doc1.profile_id)
      })
      .then((result) => {
